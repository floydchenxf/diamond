...
public class GalleryItem extends Gallery {
    ...
    public List<MoteTaskPicVO> arraylist;
    ...
    public void initAdapter(Activity context, ImageLoader imageLoader) {
        this.adapter = new ImageAdapter(context, imageLoader);
    }

    public class ImageAdapter extends BaseAdapter {
        private Activity mContext;

        private ImageLoader mImageLoarder;

        public ImageAdapter(Activity context, ImageLoader imageLoader) {
            this.mContext = context;
            this.mImageLoarder = imageLoader;
        }
        ...
        @Override
        public MoteTaskPicVO getItem(int i) {
        ...
        public View getView(int position, View convertView, ViewGroup viewgroup) {
            ...
            if (convertView == null) {
                convertView = LayoutInflater.from(mContext).inflate(R.layout.mote_task_pic_item, null);
                viewHolder = new ViewHolder();
                viewHolder.imageView = (NetworkImageView) convertView.findViewById(R.id.task_pic_item);
                convertView.setTag(viewHolder);
            }

            viewHolder = (ViewHolder) convertView.getTag();
            MoteTaskPicVO taskPicVO = getItem(position);
            if (taskPicVO != null && !TextUtils.isEmpty(taskPicVO.imgUrl)) {
                viewHolder.imageView.setImageUrl(taskPicVO.imgUrl, mImageLoarder);

                int Width = mContext.getWindowManager().getDefaultDisplay().getWidth();
                ViewGroup.LayoutParams ilp = viewHolder.imageView.getLayoutParams();
                float ondp = mContext.getResources().getDimension(R.dimen.one_dp);
                float eachWidth = (Width - 16*ondp)/3;
                if (ilp == null) {
                    ilp = new ViewGroup.LayoutParams((int)eachWidth, (int)(70*ondp));
                } else {
                    ilp.width = (int) eachWidth;
                }
            }
            return convertView;
        }
